...
const parseLine = (fns, current, index, arr) => {
  ...
  if (current[0][0] === "VariableDeclaration") {
    ...
    fns[currentfn] = { body: [], params: getParams(current[1][1]) }
    return fns
  }
...
const buildFunctionBody = fn => ({
  type: "BlockStatement",
  body: fn.body.map((current, index, array) => {
...
const getModuleName = lines => {
  const name = lines.find(line => line[0] && line[0][0] === "ModuleDeclaration")

  return name && name[1][1]
}

// module.exports = tokens => {
//   const moduleName = getModuleName(tokens)
